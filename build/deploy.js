...
function prepareZip() {
    return new Promise((success, error) => {
        ...
        archive.pipe(output);
        archive
            .directory(path.resolve(__dirname, '../dist'), false)
            .finalize();
    });
}
...
function sshUpload([zipFile, sshHandle]) {
    return new Promise((success, error) => {
        const now = new Date();
        const postfix = `${now.getFullYear()}-${now.getMonth()}-${now.getDate()}-${now.getHours()}${now.getMinutes()}`;
        const uploadedFilePath = `/home/javapro/frontend/releases/dist-${postfix}.zip`;
        sshHandle.putFile(zipFile, uploadedFilePath).then(function () {
...
function sshSymlink([uploadedFilePath, sshHandle]) {
    return new Promise((success, error) => {
        sshHandle.exec("ln -sf", [ uploadedFilePath, '/home/javapro/frontend/latest-release.zip']).then(function(result) {
